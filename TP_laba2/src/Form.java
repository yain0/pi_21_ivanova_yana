...
public class Form {
	...
	private void buttonLoc_Click() {
		...
		int place = prichal.PutNewBoatInPrichal(kut);
		panel.repaint();
		JOptionPane.showMessageDialog(frame, "Ваше место " + place);
	}

	private void buttonHeat_Click() {
		Transport kut= new NewBoat(true,true,7,300,250,color,dopColor);
		int place = prichal.PutNewBoatInPrichal(kut);
		panel.repaint();
		JOptionPane.showMessageDialog(frame, "Ваше место " + place);
	}
	...
	private void buttonGet_Click() {
			...
			try{
				Transport kut = prichal.GetNewBoatFromPrichal(Integer.parseInt(formattedTextField.getText()));
				if (kut != null) {
					kut.setPosition(5, 5);
				...
				JOptionPane.showMessageDialog(frame, "Ошибочка вышла");
			}
	}
	...
	private void initialize() {
		...
		panel = new PrichalPanel(prichal);
